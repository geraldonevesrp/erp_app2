...
interface EmpresasDataTableProps<TData, TValue> {
...
const STORAGE_KEY = 'empresas-table-column-visibility'
...
export function EmpresasDataTable<TData, TValue>({
  ...
  pageSize = 10,
  onAddClick
}: EmpresasDataTableProps<TData, TValue>) {
        ...
        <div className="flex items-center flex-1 gap-2">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input
              ...
              className="pl-8"
            />
            {(table.getColumn("razao_social")?.getFilterValue() as string)?.length > 0 && (
              <X
                className="absolute right-2 top-2.5 h-4 w-4 text-muted-foreground cursor-pointer"
                onClick={() => table.getColumn("razao_social")?.setFilterValue("")}
              />
            )}
          </div>
        </div>
